...
-record(istate,
        {top_pid                    :: pid(),
         ...
         logs = []              :: [{string(), lux:filename(), lux:filename()}],
         tail_status = []           :: [{lux:filename(), integer() | atom()}],
         ...
         want_more                  :: boolean(),
         old_want_more              :: 'undefined' | boolean(),
...
-record(cstate,
        {orig_file               :: string(),
         ...
         log_fun                 :: function(),
         log_prefix              :: string(),
         event_log_fd = closed   :: closed | {true, file:io_device()},
         stdin_log_fd = closed   :: closed | {false, file:io_device()},
         stdout_log_fd = closed  :: closed | {false, file:io_device()},
         ...
         fail = no_pattern       :: no_pattern | #pattern{},
         success = no_pattern    :: no_pattern | #pattern{},
         loop_stack = []         :: [#loop{}],
         expected                :: no_cmd | #cmd{},
         pre_expected = []       :: [#cmd{}],
         actual = <<>>           :: binary(),
         ...
         exit_status = no_exit   :: no_exit   | integer(),
         timer = no_timer        :: no_timer  | infinity | reference(),
         timer_started_at = no_timer ::
           no_timer | {non_neg_integer(), non_neg_integer(), non_neg_integer()},
         wakeup = no_wakeup      :: no_wakeup | reference(),
         debug_level = 0         :: non_neg_integer(),
         events = []             :: [tuple()],
         warnings = []           :: [#warning{}]}).
